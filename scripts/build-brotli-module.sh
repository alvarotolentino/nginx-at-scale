...
# ngx_brotli's Makefile only exposes the `modules` target (no per-.so rule), and it
# builds the filter module FIRST — whose link needs libbrotlienc (the bundled brotli
# we deliberately don't build). `make -k` keeps going past that link failure so the
# static .so (which links against NO brotli lib) still gets built.
log_step "make -k modules (filter link failure expected+ignored; static .so is the goal) — logging to $LOG"
make -k -j"$(nproc)" modules >>"$LOG" 2>&1 || true
